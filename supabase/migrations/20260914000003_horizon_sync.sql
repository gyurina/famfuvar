-- Az órarend mentése után a már létező (mai) programok fuvarjait is szinkronizálni kell.
-- Hibák:
-- 1) A trigger lehet, hogy nincs kint; a kliens RPC-t is hív (lásd refreshHorizon).
-- 2) generate_horizon a conflict ágon nem nullázta a v_occ_id-t (előző nap id-je maradt).
-- 3) current_date UTC; a nap Budapest szerint számít.
    ...
    loop
      ...
      foreach v_person_id in array v_persons loop
        ...
        if v_occ_id is not null then
          ...
          select id, coalesce(is_override, false) into v_occ_id, v_is_override
          from occurrence
