-- ╔══════════════════════════════════════════════════════════════════╗
-- ║  generate_horizon — előre generálja az occurrence + leg sorokat ║
-- ╚══════════════════════════════════════════════════════════════════╝
  ...
  while v_date <= v_end_date loop
    ...
    loop
      ...
      if v_occ_id is null then
        select id into v_occ_id from occurrence
      ...
      if r_template.needs_dropoff then
        select coalesce(
          (select t.minutes from travel_time t
           join location home on home.id = t.from_location and home.is_home and home.household_id = p_household_id
        ...
        values (
          ...
          (v_date + r_template.starts_at - make_interval(mins => v_travel_min)) at time zone 'Europe/Budapest',
      ...
      if r_template.needs_pickup then
        select coalesce(
          (select t.minutes from travel_time t
           join location home on home.id = t.to_location and home.is_home and home.household_id = p_household_id
        ...
        values (
          ...
          (v_date + r_template.ends_at + make_interval(mins => v_travel_min)) at time zone 'Europe/Budapest',
          r_template.location_id
        )
        on conflict (occurrence_id, direction) do nothing;
      end if;
